...
# KeepAChangelog PowerShell coding standards
...
- Preserve existing naming and command model conventions for public commands/functions such as `Invoke-KeepAChangelog*`, `Get-KeepAChangelog*`, `Update-KeepAChangelog*`.
- Do not create or maintain hand-written module `.psm1` or module `.psd1` files in source. Nova generates the built module root and manifest under `dist/KeepAChangelog/` from `project.json` and `src/**/*.ps1`.
...
- Private helper names should not use the public `Invoke-KeepAChangelog*`, `Get-KeepAChangelog*`, `Update-KeepAChangelog*`. Give private helpers clear implementation-focused names that describe what the helper does.
