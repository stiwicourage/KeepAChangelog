---
applyTo: "docs/KeepAChangelog/**/*.md"
---
...
Use this file when creating or updating command help under `docs/KeepAChangelog/en-US/`.
...
```powershell
$newMarkdownHelp = @{
    CommandInfo  = Get-Command -Module 'KeepAChangelog'
    OutputFolder = './docs'
    WithModulePage = $true
    Force = $true
}
...
3. Every new public entry point must add its matching command-help file in the same change. A new `src/public/<CommandName>.ps1` file is not done until `docs/KeepAChangelog/en-US/<CommandName>.md` exists.
4. Keep the resulting command-help files under `docs/KeepAChangelog/en-US/` before handoff. If you generate files in a staging folder first, move the command-help markdown files into the repository's locale folder before you finish.
...
```powershell
Measure-PlatyPSMarkdown -Path ./docs/KeepAChangelog/en-US/*.md |
...
```powershell
Test-MarkdownCommandHelp -Path ./docs/KeepAChangelog/en-US/*.md -DetailView

Import-MarkdownCommandHelp -Path ./docs/KeepAChangelog/en-US/<CommandName>.md |
...
- Files under `docs/KeepAChangelog/en-US/` must be valid PlatyPS command-help markdown, not plain project prose.
...
- Use existing valid files under `docs/KeepAChangelog/en-US/` as the structural template before inventing a new layout.
...
- Reviewers should flag help files where `external help file` contains a command name (e.g., `Get-Something-Help.xml`) instead of the module name (`KeepAChangelog-Help.xml`). This produces per-command XML files that the module manifest cannot locate at runtime.
