...
KeepAChangelog is migrating tests to a 1:1 source-to-test layout per issue #208. New and migrated tests follow this loading pattern:
...
- Importing the built `dist/KeepAChangelog` module from a new mirrored test - mirrored tests dot-source `src/**/*.ps1` files directly instead.
- Using `InModuleScope KeepAChangelog { ... }` in new tests - the function under test is already in scope after dot-sourcing.
