---
name: reviewer
description: Reviews KeepAChangelog changes for correctness, maintainability, validation, and documentation completeness
---

# KeepAChangelog reviewer agent
...
- Check that public commands/classes have matching valid PlatyPS-compatible help and that new source files have source-mirrored tests. Flag help files under `docs/KeepAChangelog/en-US/` that look like plain Markdown, break the required PlatyPS section order, or would fail `Test-MarkdownCommandHelp` / `Import-MarkdownCommandHelp`.
- Flag any new public entry point that does not add its matching help file in the same change.
- Flag help files where `external help file` contains a command name instead of the module name. The correct value is `KeepAChangelog-Help.xml`; a per-command name like `Get-Something-Help.xml` means the help was generated without the built module imported.
