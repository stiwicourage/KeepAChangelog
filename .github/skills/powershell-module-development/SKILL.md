...
description: Guidance for changing KeepAChangelog public commands, private helpers, CLI routing, packaging behavior, or project.json resolution. Use when implementing or refactoring KeepAChangelog PowerShell/module behavior.
---
...
- Do not create or maintain hand-written module `.psm1` or module `.psd1` files in source; Nova generates those files under `dist/KeepAChangelog/`.
- Preserve native PowerShell semantics. Keep Nova naming patterns on public commands, and give private helpers clear implementation-focused names instead of public-style `Invoke-KeepAChangelog*`, `Get-KeepAChangelog*`, or `Update-KeepAChangelog*` naming.
...
- Add or update valid PlatyPS-compatible help under `docs/KeepAChangelog/en-US/` when public commands or public classes change. Always build and import the dist module first (`Import-Module ./dist/KeepAChangelog/KeepAChangelog.psd1 -Force`) before running `New-MarkdownCommandHelp` or `Update-MarkdownCommandHelp`, so PlatyPS writes the module name — not the command name — into `external help file` and `Module Name`. Use `Test-MarkdownCommandHelp` to validate structure before handoff instead of writing plain Markdown from scratch.
...
- Writing plain Markdown under `docs/KeepAChangelog/en-US/` that lacks YAML metadata or the PlatyPS structure expected by `Import-MarkdownCommandHelp`
...
- Adding a new public entry point without the matching help file in `docs/KeepAChangelog/en-US/`
